...
manage_wifi() {
    ...
    local chosen_network=$(echo -e "$formatted_list" | rofi -dmenu -i -selected-row 1 -p "Wi-Fi SSID: " -theme "~/.config/rofi/themes/network.rasi")
    local ssid_index=-1
...
main_menu() {
    ...
    ##==> Выводим Rofi меню
    #######################################################
    local chosen_option=$(echo -e "$wifi_toggle$manage_wifi_btn\n󱓥 Manage Ethernet" | rofi -dmenu -p " Network Management: " -theme "~/.config/rofi/themes/network.rasi")
